...
const std::vector<std::string> LABELS = {SIZE, INIT, OPER, COPY, VERI, T0RD, T0DG, T1RD, T1DG, DRAD, DDEG};
...
std::vector<vec<double>> build_vectors(std::size_t count, std::size_t size)
{
    ...
    {
        auto norm = [seed, idx] __device__(std::size_t i) -> double
        { return normal(0.0, 1.0, seed + idx, i); };
