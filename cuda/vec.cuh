...
/// @param size Number of elements in the array to initialize
/// @param d_res Array in the device kernel to be initialized
/// @param d_fn Function to map the array index to a value to assign the
/// corresponding array element.
template <typename T, typename FUNC>
__global__ void cuda_fn_init(std::size_t size, T *d_res, FUNC d_fn)
{
    std::size_t idx = blockDim.x * blockIdx.x + threadIdx.x;
...
/// located
/// @param d_fn Function to map values of types L and R into type T in
    ...
    FUNC d_fn)
{
    ...
    if (idx < size)
    {
        d_res[idx] = d_fn(d_x[idx], d_y[idx], idx);
    }
}
...
/// @param d_fn Function to map two arguments of type T to a single argument
/// of type T. The function d_fn should be commutative
template <typename T, typename FUNC>
__global__ void cuda_step(std::size_t size, T *d_res, const T *d_src, FUNC d_fn)
{
    ...
    if (idx < span)
    {
        d_res[idx] = ((idx + span) < size) ? d_fn(d_src[idx], d_src[idx + span]) : d_src[idx];
    }
}
...
private:
    ...
    /// @param d_fn Function to map elements from source vector into this new one
    template <typename S, typename FUNC>
    vec(const vec<S> &src, FUNC d_fn)
        : vec(src._size)
    {
        cuda_map<<<this->BLOCK_COUNT, THREADS_PER_BLOCK>>>(this->_size, this->_d_vec, src._d_vec, d_fn);
    }
    ...
    /// @param d_fn Function to combine the left and right elements to compute each element of the new vector
          ...
          _d_vec(nullptr)
    {
        ...
        cuda_op<<<this->BLOCK_COUNT, THREADS_PER_BLOCK>>>(this->_size, this->_d_vec, l_src._d_vec, r_src._d_vec, d_fn);
    }
...
public:
    ...
    vec(const std::initializer_list<T> &src)
        : vec(src.size())
    {
        std::copy(src.begin(), src.end(), this->_h_vec);
        this->to_device();
    }
    ...
    /// @param d_fn Function used to initialize each new element of the vector
    template <typename FUNC>
    vec(std::size_t size, FUNC d_fn)
        : vec(size)
    {
        cuda_fn_init<<<this->BLOCK_COUNT, THREADS_PER_BLOCK>>>(size, this->_d_vec, d_fn);
    }
    ...
    virtual inline void to_host() { cudaMemcpy(this->_h_vec, this->_d_vec, this->_size * sizeof(T), cudaMemcpyDeviceToHost); }

    /// @brief Copy the content of the host to the device kernel
    virtual inline void to_device() { cudaMemcpy(this->_d_vec, this->_h_vec, this->_size * sizeof(T), cudaMemcpyHostToDevice); }
    ...
    /// @param d_fn Function used to transform source vector into destination vector
    /// @return The source function transformed on an elementwise basis to produce
    /// the destination vector
    template <typename S, typename FUNC>
    static vec<T> map(const vec<S> &src, FUNC d_fn)
    {
        return vec<T>(src, d_fn);
    }
    ...
    /// @param d_fn Function operating elementwise on each of the left and right
    ...
    T reduce(FUNC d_fn) const
    {
        ...
        while (size > 1)
        {
            ...
            cuda_step<<<this->BLOCK_COUNT, THREADS_PER_BLOCK>>>(size, d_res, d_tmp, d_fn);
            size = next_size;
...
{
    return vec<T>::template op(x, y, [] __device__(const T &l, const T &r, std::size_t idx) -> T
                               { return l + r; });
}
...
{
    return vec<T>::template map(y, [x] __device__(const T &r, std::size_t idx) -> T
                                { return x + r; });
}
...
{
    return vec<T>::template map(x, [y] __device__(const T &l, std::size_t idx) -> T
                                { return l + y; });
}
...
{
    vec<bool> cmp = vec<bool>::template op(x, y, [] __device__(const T &l, const T &r, std::size_t idx) -> bool
                                           { return l == r; });
