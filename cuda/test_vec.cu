...
std::string test_ini()
{
    ...
    vec<T> e(size, [] __device__(std::size_t idx) -> T
             { return T(idx); });
...
std::string test_map(std::size_t size)
{
    ...
    vec<T> a = vec<T>::template map(src, [TWO] __device__(T x, std::size_t idx) -> T
                                    { return TWO * x; });
    vec<T> b = vec<T>::template map(src, [THREE] __device__(T x, std::size_t idx) -> T
                                    { return THREE * x; });
    vec<T> c = vec<T>::template op(a, b, [] __device__(T x, T y, std::size_t idx) -> T
                                   { return x + y; });
